...
answer1_inspect <- function(x) {
  choices(
    x,
    a = no("The return-value of our target is not a function, but a linear model "),
    b = yes(),
    c = no("The return-value of our target is not a datafrane, but a linear model"),
    d = no("The return-value of our target is not a datafrane, but a linear model"),
    invalid()
  )
}
...
answer1_function <- function(x) {
  choices(
    ...
    c = no("A change in a downstream target does not affect an upstream target"),
    d = yes(),
    invalid()
  )
}
